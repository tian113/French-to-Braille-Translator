...
def is_all_caps(s):
    ...
    if (len(s)<2):
        ...
        for c in s:
            if (not (is_capitalized(c) and is_letter(c))):
                return False
...
def word_to_braille(word):
    ...
    .. .o .. .o .. .o .. .o o. o. ..
    '''
    #check whether letters in word are all capitalized.
    if is_all_caps(word):
        
        res=(CAPITAL+'\n\n')*2
        #this is the prefix braille for all capitalized letters.
        
        for c in word:
            
            letter_braille=char_to_braille(c)+'\n\n'
    ...
    for c in word:   
        one_char=char_to_braille(c)+'\n\n'
        
        if is_capitalized(c):
            one_char=CAPITAL+'\n\n'+one_char
            #convert capitalized letters into brialle.
                
        if is_digit(c):
            one_char=NUMBER+'\n\n'+one_char
            #convert digits into braille.
...
def file_to_braille(fname, translation_function = text_to_braille, addition = "braille"):
    ...
    True
    '''
    #note that sometimes different text editors may mess around
    #with \n and space so the results may differ
    direc = 'tests/'
